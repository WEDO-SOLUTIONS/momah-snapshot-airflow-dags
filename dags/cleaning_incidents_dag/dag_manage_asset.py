...
from include.cleaning_incidents_dag.attribute_mapper import ATTRIBUTE_MAPPER
...
def _build_schema_from_db(oracle_hook: OracleHook) -> Tuple[List[Dict], List[Dict]]:
    db_view = Variable.get("cleaning_incidents_db_view_name")

    asset_config = Variable.get("cleaning_incidents_asset_config", deserialize_json=True)
...
@dag (
    ...
    dag_id = "cleaning_incidents_manage_asset",
    start_date = datetime(2025, 1, 1),
    schedule = None,
    catchup = False,
    max_active_runs = 1,
    tags = ["urbi_pro", "cleaning_incidents", "asset_management"],
    doc_md = """
    ...
    params = {
        ...
        "asset_id": Param("", type=["null", "string"], description="Optional. If blank for UPDATE/CLEAR, the DAG will use the 'cleaning_incidents_dynamic_asset_id' Variable."),
        "db_conn_id": Param("oracle_db_conn_momrah", type="string"),
...
def manage_asset_dag():
    ...
    def execute_operation(**context):
        ...
        if op in ["UPDATE", "CLEAR_ALL_DATA"] and not asset_id:
            ...
            asset_id = Variable.get("cleaning_incidents_dynamic_asset_id", default_var=None)
        ...
        if op in ["CREATE", "UPDATE"]:
            ...
            if op == "CREATE":
                ...
                if new_asset_id and new_token:
                    Variable.set("cleaning_incidents_dynamic_asset_id", new_asset_id)
                    Variable.set("cleaning_incidents_push_data_access_token", new_token, serialize_json=False)
            ...
            Variable.set("cleaning_incidents_known_ids", [], serialize_json=True)

            log.info("SUCCESS: Cleared all data from asset and reset the 'cleaning_incidents_known_ids' state Variable.")
