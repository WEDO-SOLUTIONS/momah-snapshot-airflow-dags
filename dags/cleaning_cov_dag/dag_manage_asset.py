...
@dag (
    ...
    dag_id = "cleaning_cov_manage_asset",
    start_date = datetime(2025, 1, 1),
    schedule = None,
    catchup = False,
    max_active_runs = 1,
    tags = ["urbi_pro", "cleaning_cov", "asset_management"],
    doc_md = """
    ...
    params = {
        ...
        "asset_id": Param("", type=["null", "string"], description="Optional. If blank for UPDATE/CLEAR, the DAG will use the 'cleaning_cov_dynamic_asset_id' Variable."),
...
def manage_asset_dag():
    ...
    def execute_operation(**context):
        ...
        if op in ["UPDATE", "CLEAR_ALL_DATA"] and not asset_id:
            ...
            asset_id = Variable.get("cleaning_cov_dynamic_asset_id", default_var=None)
        ...
        if op in ["CREATE", "UPDATE"]:
            ...
            if op == "CREATE":
                ...
                if new_asset_id and new_token:
                    Variable.set("cleaning_cov_dynamic_asset_id", new_asset_id)
                    Variable.set("cleaning_cov_push_data_access_token", new_token, serialize_json=False)
