...
from include.cleaning_cov_dag.attribute_mapper import ATTRIBUTE_MAPPER
...
@dag (
    ...
    dag_id = "cleaning_cov_sync_data",
    start_date = datetime(2025, 1, 1),
    #schedule = "0 */6 * * *",
    schedule_interval="0 */6 * * *",
    catchup = False,
    max_active_runs = 1,
    tags = ["urbi_pro", "cleaning_cov", "data_sync"],
    doc_md = """
    ### Cleaning Cov Full Data Sync
    Performs a full data sync from the Oracle view to the Urbi Pro dynamic asset.
    - **Step 1**: Triggers the `cleaning_cov_manage_asset` DAG to update the asset schema.
...
def sync_data_dag():
    update_asset_schema = TriggerDagRunOperator (
        ...
        trigger_dag_id = "cleaning_cov_manage_asset",
    ...
    def get_record_count_and_generate_chunks() -> List[Dict]:
        ...
        db_view = Variable.get("cleaning_cov_db_view_name")
    ...
    def process_and_push_chunk(chunk_info: Dict):
        ...
        asset_config = Variable.get("cleaning_cov_asset_config", deserialize_json=True)
        ...
        asset_id = Variable.get("cleaning_cov_dynamic_asset_id")

        token = Variable.get("cleaning_cov_push_data_access_token")
    ...
    def process_and_push_deletes() -> List[str]:
        last_known_ids = Variable.get("cleaning_cov_known_ids", default_var=[], deserialize_json=True)
    ...
    def update_final_state(all_current_ids: List[str]):
        Variable.set("cleaning_cov_known_ids", all_current_ids, serialize_json=True)
