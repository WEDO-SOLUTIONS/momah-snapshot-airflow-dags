...
@dag (
    ...
    params = {
        ...
        "asset_id": Param("", type=["null", "string"], description="Optional. If blank for UPDATE/CLEAR, the DAG will use the 'digging_license_info_dynamic_asset_id' Variable."),
...
def manage_asset_dag():
    ...
    def execute_operation(**context):
        ...
        if op in ["UPDATE", "CLEAR_ALL_DATA"] and not asset_id:
            ...
            asset_id = Variable.get("digging_license_info_dynamic_asset_id", default_var=None)
        ...
        if op in ["CREATE", "UPDATE"]:
            ...
            if op == "CREATE":
                ...
                if new_asset_id and new_token:
                    Variable.set("digging_license_info_dynamic_asset_id", new_asset_id)
                    Variable.set("digging_license_info_push_data_access_token", new_token, serialize_json=False)
            ...
            Variable.set("digging_license_info_known_ids", [], serialize_json=True)

            log.info("SUCCESS: Cleared all data from asset and reset the 'digging_license_info_known_ids' state Variable.")
