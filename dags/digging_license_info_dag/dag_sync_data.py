...
@dag (
    ...
    dag_id = "digging_license_info_sync_data",
    start_date = datetime(2025, 1, 1),
    #schedule = "0 */6 * * *",
    schedule_interval="0 */6 * * *",
    catchup = False,
    max_active_runs = 1,
    tags = ["urbi_pro", "digging_license_info", "data_sync"],
    doc_md = """
    ### Digging License Info Full Data Sync
    Performs a full data sync from the Oracle view to the Urbi Pro dynamic asset.
    - **Step 1**: Triggers the `digging_license_info_manage_asset` DAG to update the asset schema.
...
def sync_data_dag():
    update_asset_schema = TriggerDagRunOperator (
        ...
        trigger_dag_id = "digging_license_info_manage_asset",
    ...
    def process_and_push_chunk(chunk_info: Dict):
        ...
        asset_id = Variable.get("digging_license_info_dynamic_asset_id")

        token = Variable.get("digging_license_info_push_data_access_token")
    ...
    def process_and_push_deletes() -> List[str]:
        last_known_ids = Variable.get("digging_license_info_known_ids", default_var=[], deserialize_json=True)
