...
def validate_and_convert_row(row: Dict[str, Any], primary_name_column: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        vpi_num = float(properties.get('vpi', 0))
    ...
    if vpi_num == 0:
        ...
        properties['vpi'] = vpi_num

    try:
        cov_num = float(properties.get('coverage', 0))
    except (TypeError, ValueError):
        cov_num = 0
    if cov_num == 0:
        properties['coverage'] = 0.1
    else:
        properties['coverage'] = cov_num
...
@dag (
    ...
    dag_id = "gov_high_low_sync_data",
    start_date = datetime(2025, 1, 1),
    #schedule = "0 */6 * * *",
    schedule_interval="0 */6 * * *",
    catchup = False,
    max_active_runs = 1,
    tags = ["urbi_pro", "gov_high_low", "data_sync"],
    doc_md = """
    ### Gov High Low Full Data Sync
    Performs a full data sync from the Oracle view to the Urbi Pro dynamic asset.
    - **Step 1**: Triggers the `gov_high_low_manage_asset` DAG to update the asset schema.
...
def sync_data_dag():
    update_asset_schema = TriggerDagRunOperator (
        ...
        trigger_dag_id = "gov_high_low_manage_asset",
    ...
    def process_and_push_chunk(chunk_info: Dict):
        ...
        asset_id = Variable.get("gov_high_low_dynamic_asset_id")

        token = Variable.get("gov_high_low_push_data_access_token")
    ...
    def process_and_push_deletes() -> List[str]:
        last_known_ids = Variable.get("gov_high_low_known_ids", default_var=[], deserialize_json=True)
    ...
    def update_final_state(all_current_ids: List[str]):
        Variable.set("gov_high_low_known_ids", all_current_ids, serialize_json=True)
