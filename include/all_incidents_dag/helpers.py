# include/all_incidents_dag/helpers.py
...
def validate_and_convert_row(
    ...
    if geom_dim == "point":
        try:
            ...
            log.warning(f"Skipping record {ci.get('id')} due to invalid coordinates for point")
            return None
        ...
        # Build LineString from explicit start/end lon/lat columns
        try:
            start_lon = float(ci.get("start_point_longitude"))
            start_lat = float(ci.get("start_point_latitude"))
            end_lon   = float(ci.get("end_point_longitude"))
            end_lat   = float(ci.get("end_point_latitude"))
        except Exception:
            log.warning(
                f"Skipping record {ci.get('id')}: "
                "missing or invalid start/end coordinates for line"
            )
            return None
        geometry = {
            "type": "LineString",
